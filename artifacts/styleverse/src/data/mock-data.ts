...
// Add more generic products by cloning and varying data slightly
for (let i = 7; i <= 60; i++) {
  const baseProduct = MOCK_PRODUCTS[i % 6];
  MOCK_PRODUCTS.push({
    ...baseProduct,
    id: `p${i}`,
    name: `${['Vintage', 'Modern', 'Essential', 'Premium', 'Signature'][i % 5]} ${baseProduct.name}`,
    price: baseProduct.price + (i * 10),
    mrp: baseProduct.mrp + (i * 20),
    rating: 3.5 + (i % 15) / 10,
    reviewCount: 10 + (i * 13) % 2000,
  });
}
